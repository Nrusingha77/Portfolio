...
function SpringElement({
  ...
  React.useEffect(() => {
    ...
    const unsubscribeX = springX.on('change', updatePath);
    const unsubscribeY = springY.on('change', updatePath);
  ...
  }, [center, springX, springY, springPathConfig]);
  ...
  return (
    <>
      <svg
        width="100vw"
        height="100vh"
        className="fixed w-screen h-screen pointer-events-none inset-0">
        <path
          ref={pathRef}
          strokeLinecap="round"
